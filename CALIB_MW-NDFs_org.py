...
# This script measures currents in dark and illuminated conditions for each filter position.
# Highest measurable optical power with filter on OPM be in mW order (visible spectrum of EM).
# Lowest measurable optical power without filter on OPM be in 0.1nW order or higher (visible spectrum of EM).
...
number_of_points = 5 # number of points of optcal power measurement at each filter-pair combination
measurement_interval = 1  # time period (in s) between adjacent datapoints of optical power, per filter-conmbination
WL = 407 # enter peak wavelength in nm. Script assumes the incident light to be monochromatic.
...
WH.move(WH.move(150))   # Move to dark position; position number 7-1 (no filter in this slot; only opaque)
...
# GOOD - Save Optical power recorded in Dark condition (0-0)
file_name = f"Results dump/RawData_PM-t_{measurement_name}_{number_of_points}pts_{measurement_interval}s_0-0.csv"
...
print("Data for dark condition saved.")  # status update
# *******************************************************************

# MEASUREMENT OF MOTORIZED WHEEL's FILTERS START HERE !!
WH.move(WH.move(-87))   # Move to dark position
WH.move(WH.move(125))   # Move to dark position
print("Now starting for different filter combinations.")  # status update
file_path = "Wheel_Calibration_Sid_All.txt"
